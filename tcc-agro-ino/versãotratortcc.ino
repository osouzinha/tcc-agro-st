...
#define ID_TRATOR "Pulverizador_Trator" 
...
#define PIN_SENSOR_FLUXO_ESQ 14 // Mangueira Esquerda
#define PIN_SENSOR_FLUXO_DIR 12 // Mangueira Direita
#define LED_STATUS 2 
...
// Variáveis separadas para cada sensor
volatile long pulsosContadosEsq = 0;
volatile long pulsosContadosDir = 0;

float vazaoLPM_Total = 0.0, velocidadeKPH = 0.0, taxaAplicacao = 0.0;
...
// Funções de interrupção independentes
void IRAM_ATTR contarPulsosEsq() { pulsosContadosEsq++; }
void IRAM_ATTR contarPulsosDir() { pulsosContadosDir++; }

void gravarInterno() {
  File file = LittleFS.open("/historico.csv", "a"); 
  if (!file) return;
  String linha = String(millis()/1000) + ";" + String(vazaoLPM_Total, 1) + ";" + String(velocidadeKPH, 1) + ";" + String(taxaAplicacao, 1) + ";";
...
void handleDados() {
  String json = "{";
  json += "\"vazao\":" + String(vazaoLPM_Total, 1) + ",";
...
void handleRoot() {
  ...
  html += "<p>Vazao Total: <b id='val_vazao'>" + String(vazaoLPM_Total, 1) + "</b> L/min</p>";
...
void setup() {
  ...
  // Declaração dos dois pinos de interrupção
  pinMode(PIN_SENSOR_FLUXO_ESQ, INPUT_PULLUP);
  attachInterrupt(digitalPinToInterrupt(PIN_SENSOR_FLUXO_ESQ), contarPulsosEsq, FALLING);
  
  pinMode(PIN_SENSOR_FLUXO_DIR, INPUT_PULLUP);
  attachInterrupt(digitalPinToInterrupt(PIN_SENSOR_FLUXO_DIR), contarPulsosDir, FALLING);
...
void loop() {
  ...
  if (millis() - tempoAnterior > 1000) {
    // Cálculo individual e soma de vazão
    float vazaoLPM_Esq = (float)pulsosContadosEsq / fatorCalibracao;
    float vazaoLPM_Dir = (float)pulsosContadosDir / fatorCalibracao;
    vazaoLPM_Total = vazaoLPM_Esq + vazaoLPM_Dir;

    if (gps.location.isValid()) velocidadeKPH = gps.speed.kmph(); else velocidadeKPH = 0.0;
    
    // Cálculo da taxa com base na vazão total
    if (velocidadeKPH > 0.5) taxaAplicacao = (vazaoLPM_Total * 600.0) / (velocidadeKPH * larguraFaixa);
    else taxaAplicacao = 0.0;
    
    gravarInterno();
    
    Serial.print("["); Serial.print(ID_TRATOR); Serial.print("] ");
    Serial.print("Vazao Total: "); Serial.print(vazaoLPM_Total);
    Serial.print(" | Meta: "); Serial.println(metaTaxaAlvo);

    if (firebaseOnline && Firebase.ready()) {
       String caminho = "/frota/" + String(ID_TRATOR);

       // Envia a vazão total para o banco
       Firebase.setFloat(fbdo, caminho + "/vazao", vazaoLPM_Total);
    ...
    // Zera os contadores separadamente
    pulsosContadosEsq = 0;
    pulsosContadosDir = 0;
